...
st.markdown(
    "Compare different scenarios and shortlists side by side. "
    "Save scenarios from the **Scenarios** page or from **Likelihood and Prioritization** to compare them here."
)
...
if len(available) < 2:
    st.warning(
        ...
        "Run Stage 1 (Filtering) and Stage 2 (Likelihood and Prioritization) first, "
        "or save scenarios from there."
    )
...
with st.sidebar:
    ...
    if has_quadrant_source:
        ...
        st.info("Run **Likelihood and Prioritization** first to assign quadrants.")
...
# TABS
# ============================================================
tab_side, tab_radar, tab_overlap = st.tabs(["Side-by-Side", "Radar Chart", "Overlap Table"])
...
with tab_overlap:
    ...
    # Coalesce identity/metadata columns duplicated by the outer merge
    for base_col in ["hs_product_code", "description", "hs2_name", "global_export_value"]:
        col_a_name = f"{base_col} ({name_a})"
        col_b_name = f"{base_col} ({name_b})"
        if col_a_name in merge_df.columns and col_b_name in merge_df.columns:
            merge_df[base_col] = merge_df[col_a_name].fillna(merge_df[col_b_name])
            merge_df = merge_df.drop(columns=[col_a_name, col_b_name])
